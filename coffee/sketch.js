...
wp = (p) => {
  ...
  // grid.p[1] = -grid.p[1]
  dump(`${gpsCount}  ${round(bearingBetween(matrix.p, matrix[target]))} ${target} ${round(distanceBetween(matrix.p, matrix[target]), 1)} ${round(p.coords.latitude, 6)} ${round(p.coords.longitude, 6)} `);
